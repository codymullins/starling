...
/// <summary>
/// The chrome icon set — a direct port of the <c>ICONS</c> path-data table in
/// <c>design/chrome.jsx</c>. Each entry is an SVG path string on a 16×16 grid,
/// stroked at 1.5px; <see cref="Make"/> turns one into a MAUI <see cref="Path"/>.
/// </summary>
public static class Icons
{
    ...
    /// thickness scales with <paramref name="size"/> so the icon's visual
    /// weight matches the design at any size (the JSX scales the whole SVG).
    /// </summary>
    public static Path Make(string data, Color stroke, double size = 16, double strokeWidth = 1.5)
    {
        var geometry = (Geometry)new PathGeometryConverter().ConvertFromInvariantString(data)!;
        return new Path
        {
            Data = geometry,
